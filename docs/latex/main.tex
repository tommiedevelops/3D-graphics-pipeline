...
\date{}
\title{Mini Graphics Engine, in Pure C}
...
\begin{itemize}
	\item Thanks for deciding to read this document! I hope you find it useful and informative. If you have any questions, comments or suggestions, please feel free to reach out to me at thomasjdizon@gmail.com.

\item I tend to like writing in a casual and conversational style, so I hope that makes the content more approachable and easier to understand. 
\item I also prefer to write with dot points instead of conventional paragraphs as I find it makes it easier to separate points of concern and makes the content more digestible.

\item This project is a work in progress and I will be updating this document as I continue to work on the project. So if you find any errors or have any suggestions for improvement, please let me know!
\end{itemize}
...
\begin{itemize}
	\item The goal here is to represent 3D objects on a computer, manipulate those objects in interesting ways and finally render those objects to the screen. 
	\item The mental model I would like to build upon is like that in Figure \ref{fig:mental-model-1}. We imagine some computer memory, a screen to render onto and a processing unit which is capable of reading and manipulating memory adn writing to the screen.
\end{itemize}
...
\begin{figure}[htbp]
	...
	\begin{tikzpicture}[]
		\filldraw[draw=black, fill=white] (2,0) rectangle (\textwidth,5);
	\end{tikzpicture}

	\caption{A mental model of a graphics engine. The programmer interacts with the graphics engine through an API which allows them to create and manipulate 3D objects in memory and then render those objects to the screen. The graphics engine is responsible for managing the memory of the 3D objects, performing computations on those objects and ultimately rendering them to the screen.}
	\label{fig:mental-model-1}
\end{figure}

\subsection{Why build it from scratch?}
\begin{itemize}
...
\subsection{How 3D Objects Are Stored}
\begin{itemize}
